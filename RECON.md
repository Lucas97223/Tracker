...
> 4. **Confirmed against production during the hand deploy (2026-07-08):** the live database has 0001–0006 + 0009 + 0010 applied and **never received 0007/0008** — the ledger engine existed only in the repo. (This is precisely the "0010 predates 0008" world of #3: the $0 sync worked on live because there was no GL to violate.) Deploy order is therefore 0007 → 0008 → 0011…0021; `scripts/simulate-live-deploy.sh` rehearses exactly that against production-shaped data, and the deploy-only fixes it forced are recorded in commit `27d4175` (zero-amount expenses post nothing; structural org backfill vs. immutability/deferred-constraint interactions in 0011).
